...
namespace Graphics_StickDrawing
{
    public partial class Form1 : Form
    {
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            {
                int height = 100 * percentile[i] / sumPercentile;
                Color color = Color.FromArgb(random.Next(255), random.Next(255), random.Next(255));
                
                HatchBrush brush = new HatchBrush((HatchStyle)random.Next(50), color);
                graphics.FillRectangle(brush, 0, 80 + i * 20, height, 18);
                graphics.DrawString("% " + height.ToString(), new Font("Tahoma", 8, FontStyle.Regular), 
                                    new SolidBrush(Color.Red), height + 5, 80 + i * 20);
